...
class MultiDayTourController extends Controller{
    public function index(Request $request){
        if ($request->ajax()) {
            $data = Hotal::select('hotels.*','locations.name')
                    ...
                    ->Where('hotels.tour_type', '=', 'multi-day-tour')
                    ->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $url =  route('admin.multidaytour.destroy');
                    $btn = '<a href=" ' . route('admin.multidaytour.show') .'/' .  \App\Helpers::encrypt($row->id) . ' " class="btn btn-primary btn-sm">View</a>
                            <a href="' . route('admin.multidaytour.edit', \App\Helpers::encrypt($row->id)) . '" class="btn btn-warning" title="Edit"><i class="mdi mdi-pencil"></i></a>
        ...
        return view('admin.multidaytour.index');
    }

    public function create(){
        $locations = Destination::all();
        return view('admin.multidaytour.create', compact('locations'));
    }
    ...
    public function store(Request $request){
        ...
        $input['tour_type'] = 'multi-day-tour';
        $input['description'] = strip_tags($input['description']);
        $hotal = Hotal::create($input);
        
        return redirect()->route('admin.multidaytour.index')->with('success', 'Hotal added successfully.');
    }
    ...
    public function edit($id){
        ...
        $hotalDetail =Hotal::select('hotels.*','locations.name')
                    ...
                    ->where('hotels.id', \App\Helpers::decrypt($id))
                    ->first();
        return view('admin.multidaytour.edit', compact('hotalDetail','id','locations'));
    }
    ...
    public function update(Request $request, $id){
       $input = $request->all();
        ...
        Hotal::where('id', \App\Helpers::decrypt($id))->update($updateHotalInfo);
          ...
          return redirect()->route('admin.multidaytour.index')
    ...
    public function destroy(Request $request){
        ...
        $hotal->delete();
        return redirect()->route('admin.multidaytour.index')->with('message', 'Hotal deleted successfully.');
    }
}