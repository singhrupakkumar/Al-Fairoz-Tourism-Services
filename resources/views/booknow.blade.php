						...
						<div class="col-md-8 col-sm-12">
							...
							<div class="tj-tabs">
								<ul class="nav nav-tabs" role="tablist">
									<li id="vehicle_type" class="{{ ($type == 'vehicle') ? 'active' : '' }}"><a href="#point" data-toggle="tab">Point to Point</a></li>
									<li id="hotal_type" class="{{ ($type == 'hotal') ? 'active' : '' }}"><a href="#hotalBooking" data-toggle="tab">Tour Booking</a></li>
								</ul>
							</div>
							<div class="tab-content">
								<div class="tab-pane {{ ($type == 'vehicle') ? 'active' : '' }}" id="point">
									<form class="booking-frm" action="{{route('home.confirm_booking')}}" method="POST" id="ride-booking-form" novalidate="novalidate">
										@csrf
										...
										<div class="col-md-12 col-sm-12">
											...
											<div class="field-holder form-group">
												...
												<select name="pickup_loc" id="pickup_start_loc" class="selectpicker">
														...
														@foreach($city as $val)
														<option data-pickup="{{$val->name}}" value="{{$val->id}}" {{ ($val->id == $formLoc) ? 'selected' : '' }} >{{$val->name}}</option>
														@endforeach
												</select>
										...
										<div class="col-md-12 col-sm-12">
											...
											<div class="field-holder form-group">
												...
												<select name="dropoff_loc" id="pickup_end_loc" class="selectpicker">
														...
														@foreach($city as $val)
														<option data-pickup="{{$val->name}}" value="{{$val->id}}" {{ ($val->id == @$toLoc) ? 'selected' : '' }} >{{$val->name}}</option>
														@endforeach
												</select>
										...
										<div class="col-md-12 col-sm-12">
											<div class="field-holder form-group">
												...
												@endphp
													...
													<select name="car_id" id="car_list" class="selectpicker" {{$disabled}}>
														<option value="">Select Car</option>
														<option value="Nissan Vela" data-hrrate="30" data-dayrate="150">Nissan Vela</option>
														@foreach(@$vehicleName as $val)
														<option data-car="{{$val->id}}" data-hrrate="{{$val->name}}" data-dayrate="{{$val->name}}" value="{{$val->name}}" {{ ($val->id == @$vehicle_id) ? 'selected' : '' }} >{{$val->name}}</option>
														@endforeach
													</select>
													<input type="hidden" name="vehicle_id" value="{{(@$vehicle_id) ? $vehicle_id : '1'}}" id="car_id">
												</div>
											</div>
										</div>
										<div class="col-md-12 col-sm-12">
								...
								<div class="tab-pane {{ ($type == 'hotal') ? 'active' : '' }}" id="hotalBooking">
									<form class="booking-frm" action="{{route('home.confirm_booking')}}" method="POST" id="ride-hotal-booking-form" novalidate="novalidate">
										@csrf
										
										<input type="hidden" name="bookingType" value="{{ isset($tour_type) ? $tour_type : 'tour' }}" >
										<div class="col-md-12 col-sm-12">
											<div class="field-holder form-group">
												<select name="hotal_id" id="select_hotal" class="selectpicker">
														<option value="">Select Tour</option>
														@foreach($hotals as $val)
														<option data-hotal="{{$val->hotal_name}}" value="{{$val->id}}" {{ ($val->id == @$hotal_id) ? 'selected' : '' }} >{{$val->hotal_name}}</option>
														@endforeach
												</select>
											</div>
										</div>
										<div class="col-md-12 col-sm-12">
											<div class="field-holder form-group">
												<select name="hotal_location" id="select_hotal_loc" class="selectpicker">
														...
														@foreach($city as $val)
														<option data-hotalLoc="{{$val->name}}" value="{{$val->id}}" {{ ($val->id == @$results->location_id) ? 'selected' : '' }} >{{$val->name}}</option>
														@endforeach
												</select>
											</div>
										</div>
